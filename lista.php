...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <div id="cont">
            ...
            ?>
        <form method="post" action="accform/accOrdenCompraEliminar.php">
            <?php
                While($Registro=$oOrdenCompra->Selecciona()){
            ?>
            <input type="checkbox" name=elimina<?=$Registro->getPIdOC()?> value="<?=$Registro->getPIdOC()?>">
                    <?=$Registro->getPFechaEmision()?>/<?=$Registro->getPTotalOC()?>/<?=$Registro->getPEstado()?>/<?=$Registro->getPIdUsuario()?>
                    <br>
            <?php
            }
            ?>
